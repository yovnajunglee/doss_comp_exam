...
run_covdynglg_temp <- function(M=20000){
  ...
  lverok1 = LogVeroLogLamb1(loglamb1k,distS,ak,alphak,nu1k)
  ...
  system.time(for (k in 1:M){
    ...
    for (j in 1:4){
      ...
      lverok = LogVeroNG(z-epsk,distS,Plat,Plong,Ptemp,d0k,d1k,d2k,d3k,d4k,sig2k,ak,alphak,loglamb1k,loglamb2k,tau2k)   ### d0k,d1k,d2k,d3k
      lverok1 = LogVeroLogLamb1(loglamb1k,distS,a1k,alpha1k,nu1k)
      ...
      lveroprop1 = LogVeroLogLamb1(loglamb1prop,distS,a1k,alpha1k,nu1k)
    ...
    lverok1 = LogVeroLogLamb1(loglamb1k,distS,a1k,alpha1k,nu1k)
    lveroprop1 = LogVeroLogLamb1(loglamb1k,distS,a1prop, alpha1k, nu1prop)
    ...
    # 
    Wlamb.out = nu1k*exp(-distS.out/a1k)
    Winv.out = solve(Wlamb.out)
    Wlamb.p.out = nu1k*exp(-distS.tot/a1k)[1:Inew, (Inew+1):(Inew + length(Plat.out))]
    a = loglamb1k + 0.5*nu1k #- (X.lamb1%*%t(betak))
    Wlamb= nu1k*exp(-distS/a1k)  ## covariance matrix for lambda
    Winv<- solve(Wlamb)
