...
## Fit CovDynGLG with Matern correlation function
## Code adapted from Fonseca et al. (2023) 
## https://github.com/thaiscofonseca/DynGLG

run_covdynglg_matern <- function(M=20000, nu){
  
  theta= matrix(0,M,10)  ## sig2, phi, alpha, nu, like, like2
  ...
  nu1k = 0.1 # eq. (3)
  #nu1k = 0
  loglamb1k = runif(Inew)# rep(0,Inew)
  nu2k = 0.1 # eq. (4a)
  #nu2k = 0
  mutk = rep(0,J) ## ??
  loglamb2k = rep(0,J)
  ...
  for (j in 1:J){
    Ftt[[j]] = t(cbind(rep(1,Inew), Plat, Plong, Ptemp[j,])) ##  n x r
  }
  R = dim(Ftt[[1]])[1]
  ...
  Ft1 = NULL
  Ft1 = t( cbind(apply(Ptemp,1,mean))) ##  n x r
  
  
  Ft1.pred = t(cbind(apply(Ptemp.p,1,mean)))
  ...
  lverok = LogVeroNG(z-epsk,distS,Plat,Plong,Ptemp,d0k,d1k,d2k,d3k,d4k,sig2k,ak,alphak,loglamb1k,loglamb2k,tau2k)   ### d0k,d1k,d2k,d3k
  
  # Log-likelihood for log(lambda_1)
  # Regression for spatial mixing process
  kappak = 10 # init for matern
  nuk = nu
  lverok1 = LogVeroLogLamb1maternfields(loglamb1k,distS,kappak,nuk,nu1k)
  
  # loglikelihood for lambda_2 temporal ( still dont know what mu2 is )
  # mutk = F2*eta
  ...
  system.time(for (k in 1:M){
    ...
    for (j in 1:J){
      mu = dt[1,1,j] +  dt[2,1,j]*Plat +  dt[3,1,j]*Plong +  dt[4,1,j]*Ptemp[j,]
    ...
    mutk<- mu.post[k,] #mean func in temporal mixing model
    ...
    for (j in 1:4){
      ...
      lverok = LogVeroNG(z-epsk,distS,Plat,Plong,Ptemp,d0k,d1k,d2k,d3k,d4k,sig2k,ak,alphak,loglamb1k,loglamb2k,tau2k)   ### d0k,d1k,d2k,d3k
      lverok1 = LogVeroLogLamb1maternfields(loglamb1k,distS,kappak,nuk,nu1k)
      lveroprop = LogVeroNG(z-epsk,distS,Plat,Plong,Ptemp,d0k,d1k,d2k,d3k,d4k,sig2k,ak,alphak,loglamb1prop,loglamb2k,tau2k);    ### d0k,d1k,d2k,d3k
      lveroprop1 = LogVeroLogLamb1maternfields(loglamb1prop,distS,kappak,nuk,nu1k)
    ...
    for (j in 1:9){
      ...
      lverok = LogVeroNGt(z-epsk,distS,Plat,Plong,Ptemp,d0k,d1k,d2k,d3k,d4k,sig2k,ak,alphak,loglamb1k,loglamb2k,tau2k,regiont)   ### d0k,d1k,d2k,d3k
      lverok2 = LogVeroLogLamb2(loglamb2k[regiont],nu2k,mutk[regiont])
      lveroprop = LogVeroNGt(z-epsk,distS,Plat,Plong,Ptemp,d0k,d1k,d2k,d3k,d4k,sig2k,ak,alphak,loglamb1k,loglamb2prop,tau2k,regiont);    ### d0k,d1k,d2k,d3k
    ...
    lverok = LogVeroNG(z-epsk,distS,Plat,Plong,Ptemp,d0k,d1k,d2k,d3k,d4k,sig2k,ak,alphak,loglamb1k,loglamb2k,tau2k)
    lveroprop = LogVeroNG(z-epsk,distS,Plat,Plong,Ptemp,d0k,d1k,d2k,d3k,d4k,sig2k,aprop,alphaprop,loglamb1k,loglamb2k,tau2k)
    ...
    ### Metropolis-Hastings for nu, nu1 and kappa
    
    kappaprop = (exp(log(kappak)+raa*rnorm(1)));
    nuAux = log(nuk/(2.5-nuk))+ rnu*rnorm(1); # force nuk to lie btw 0 and 2.5
    nuprop =  2.5*exp(nuAux)/(1+exp(nuAux));
    nu1prop = exp(log(nu1k)+rnu1*rnorm(1));
    
    lverok1 = LogVeroLogLamb1maternfields(loglamb1k,distS,kappak,nuk,nu1k)
    lveroprop1 = LogVeroLogLamb1maternfields(loglamb1k,distS,kappaprop,nuprop, nu1prop)
    
    
    auxprop = lveroprop1+log(nu1prop) +  log(dgamma(nu1prop,1,5)) +
      log(kappaprop)+ log(dgamma(kappaprop,1,2.3/median(distS))) +
      log(nuprop*(2.5-nuprop)) 
    #
    auxk = lverok1+log(nu1k)+   log(dgamma(nu1k,1,5)) +
      log(kappak) +  log(dgamma(kappak,1,2.3/median(distS))) +
      log(nuk*(2.5-nuk)) 
    ...
    auxprop = lveroprop2+log(nu2prop)+  log(dgamma(nu2prop,1,5))    
    auxk = lverok2+log(nu2k)+   log(dgamma(nu2k,1,5)) 
    ratio = auxprop-auxk;   test = runif(1);
    if (ratio>log(test)) {
      nu2k = nu2prop;
      lverok2 = lveroprop2;
    ...
    for (hh in 1:h){
      mu = theta.h.post[k,1,hh] +  theta.h.post[k,2,hh]*Plat +  theta.h.post[k,3,hh]*Plong +
        theta.h.post[k,4,hh]*Ptemp.p[hh,]
    ...
    Wlamb.out = Matern(distS.out, range = kappak, smoothness = nuk, phi = nu1k)
    #nu1k*exp(-distS.out/a1k)
    Winv.out = solve(Wlamb.out)
    Wlamb.p.out = Matern(distS.tot, range = kappak, smoothness = nuk, phi = nu1k)[1:Inew, (Inew+1):(Inew + length(Plat.out))]
    #nu1k*
    #exp(-distS.tot/a1k)[1:Inew, (Inew+1):(Inew + length(Plat.out))]
    a = loglamb1k + 0.5*nu1k #- (X.lamb1%*%t(betak))
    Wlamb= Matern(distS, range = kappak, smoothness =  nuk, phi = nu1k)
    #nu1k*exp(-distS/a1k)  ## covariance matrix for lambda
    Winv<- solve(Wlamb)
    mu.loglamb.out = -nu1k/2*rep(1, dim(distS.out)[1]) +
