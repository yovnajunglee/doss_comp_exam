...
run_covdynglg_matern <- function(M=20000, nu){
  ...
  nu1k = 0.1 # eq. (3)
  #nu1k = 0
  loglamb1k =  runif(Inew,0,1) #rep(0,Inew)
  nu2k = 0.1 # eq. (4a)
  #nu2k = 0
  mutk = rep(0,J)
  loglamb2k = rep(0,J)
  ...
  # guess: mutk intercept only no covariates  
  # mutk = F2*eta
  ...
  system.time(for (k in 1:M){
    ...
    Wlamb.out = Matern(distS.out, range = kappak, smoothness = nuk, phi = 1)
    Winv.out = solve(Wlamb.out)
    Wlamb.p.out = Matern(distS.tot, range = kappak, smoothness = nuk, phi = 1)[1:Inew, (Inew+1):(Inew + length(Plat.out))]
    a = loglamb1k + 0.5*nu1k #- (X.lamb1%*%t(betak))
    Wlamb= Matern(distS, range = kappak, smoothness =  nuk, phi = 1)
    Winv<- solve(Wlamb)
