...
LogVeroNG = function(vecz,distanceS,Plat,Plong,Ptemp,Pws,d0,d1,d2,d3,d4,sig2,phi,alpha,loglamb1,loglamb2, tau2){
  I = length(loglamb1)
  J = length(loglamb2)
  lvero=NULL
  det<- NULL
  ...
  for(t in 1:J){
    ...
    Sig2inv = lamb2t*Cinv
    mu<- d0[t] +  d1[t]*Plat +  d2[t]*Plong +  d3[t]*Ptemp[t,]+  d4[t]*Pws[t,]
...
LogVeroNGt = function(vecz,distanceS,Plat,Plong,Ptemp,Pws,d0,d1,d2,d3,d4,sig2,phi,alpha,loglamb1,loglamb2, tau2,region){
...
LogVeroLogLamb1maternreg = function(loglamb1,distanceS,kappa,nu,nu1,beta1,x1){ 
  ...
  I1 = length(loglamb1)
  lverolamb<- NULL
  Wlamb = matern.covariance(distanceS, kappa, nu, sqrt(nu1))
